        ...
        >
          ...
          <div className="px-5 pb-6">
            <div
              ...
              style={{ background: "var(--pink-bg)", border: "1px solid var(--pink-border)" }}
            >
              <img
                  ...
                  {navItems.map((item) => (
                    <button
                      ...
                      style={{
                        color: page === item.id ? "var(--pink-dark)" : "var(--muted)",
                        background:
                          page === item.id ? "var(--pink-bg)" : "transparent",
                      }}
                    >
                      {item.icon}
  ...
  return (
    <div className="card animate-fadeIn flex flex-col">
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <p className="text-sm font-semibold leading-tight mt-1 flex-1" style={{ color: "var(--charcoal)" }}>
          {product.title}
        </p>
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-sm animate-scaleIn">
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold" style={{ color: "var(--charcoal)", fontFamily: "Georgia,serif" }}>
            Musa
          </h1>
        ...
        <div className="rounded-3xl p-6 shadow-soft" style={{ background: "white", border: "1px solid var(--pink-border)" }}>
          <h2 className="text-xl font-bold mb-5" style={{ color: "var(--charcoal)", fontFamily: "Georgia,serif" }}>
            Welcome back
          </h2>
          {err && (
            <p className="text-sm mb-4 px-3 py-2 rounded-xl" style={{ color: "var(--bark)", background: "rgba(255,212,212,0.45)" }}>
              {err}
            </p>
          )}
    ...
    <div className="min-h-screen flex flex-col items-center justify-center p-6">
      <div className="w-full max-w-sm animate-scaleIn">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold" style={{ color: "var(--charcoal)", fontFamily: "Georgia,serif" }}>
            Join Musa
          </h1>
        </div>
        ...
        <p className="text-center text-sm mt-5" style={{ color: "var(--muted)" }}>
          ...
          <button onClick={() => setPage("login")} className="font-semibold hover:underline" style={{ color: "var(--sage)" }}>
            Sign in
          </button>
        </p>
      </div>
  ...
  return (
    <div>
      ...
      <div className="px-6 pt-5">
        {/* Fixed search bar — icon and placeholder no longer overlap */}
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-3 mb-4">
          {CATEGORIES.map((c) => (
            <button key={c} onClick={() => setCat(c)} className={`tag${c === cat ? " active" : ""}`}>
              {c}
            </button>
          ))}
        </div>
      ...
      <div className="px-6 pt-5 pb-8">
        {cart.length === 0 ? (
          ...
          <>
            <div className="space-y-3 mb-5">
              {cart.map((item: CartItem) => (
                <div key={item.id} className="card flex gap-4 p-4 animate-fadeIn">
                  ...
                  <div className="flex-1 min-w-0">
                    <p style={{ fontSize: "10px", color: "var(--muted)", textTransform: "uppercase" }}>{item.studio}</p>
                    <p className="text-sm font-semibold truncate" style={{ color: "var(--charcoal)" }}>{item.title}</p>
                    <div className="flex items-center justify-between mt-3">
                      <div className="flex items-center gap-3 rounded-xl px-3 py-1.5" style={{ background: "var(--parchment)" }}>
                  ...
                  <button onClick={() => removeFromCart(item.id)} className="self-start" style={{ color: "var(--muted)" }}>
                    <X size={16} />
                  </button>
                </div>
              ))}
            ...
            {/* Gift wrap — cost shown in banner, and correctly reflected in summary */}
            <div
            ...
            >
              ...
              >
                {giftWrap ? <><Check size={13} /> Added</> : <><Plus size={13} /> ${GIFT_WRAP_COST.toFixed(2)}</>}
              </button>
            </div>

            {shipping > 0 && (
              <div className="rounded-2xl p-4 mb-5 flex items-center gap-3" style={{ background: "rgba(255,212,212,0.42)" }}>
                <span>🚚</span>
            ...
            <div className="rounded-2xl p-5 mb-5 shadow-card" style={{ background: "white", border: "1px solid var(--pink-border)" }}>
              <p className="text-base font-bold mb-4" style={{ color: "var(--charcoal)" }}>Order Summary</p>
            ...
            <p className="text-center text-xs mt-3" style={{ color: "var(--muted)" }}>🔒 Secure Checkout</p>
          </>
        )}
      </div>
    </div>
...
function StudioRegistration({ setPage }: { setPage: (p: Page) => void }) {
  ...
  const [form, setForm] = useState({ studioName: "", craftType: "", description: "" });
  ...
  return (
    <div>
      ...
      <div className="px-5 py-5">
        <div className="rounded-[2rem] overflow-hidden shadow-soft border" style={{ background: "white", borderColor: "var(--pink-border)" }}>
          <div className="grid lg:grid-cols-[0.95fr_1fr] min-h-[430px]">
            <div className="relative hidden lg:block">
              ...
              />
              <div className="absolute inset-0" style={{ background: "linear-gradient(90deg, rgba(31,33,24,0.35), rgba(31,33,24,0.05))" }} />
              <div className="absolute left-6 bottom-6 max-w-xs">
                <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "var(--sage-light)" }}>Start selling</p>
                <h2 className="text-3xl font-bold leading-tight mb-2" style={{ color: "var(--cream)", fontFamily: "Georgia,serif" }}>
            ...
            <div className="p-5 md:p-7 flex items-center">
              <div className="w-full max-w-sm mx-auto">
                <div className="w-11 h-11 rounded-2xl flex items-center justify-center mb-4" style={{ background: "var(--pink-bg)" }}>
                  <Store size={22} style={{ color: "var(--pink-dark)" }} />
                </div>
                <h1 className="text-3xl font-bold mb-1" style={{ color: "var(--charcoal)", fontFamily: "Georgia,serif" }}>
                ...
                <div className="space-y-3">
                  <input className="input-field" placeholder="Studio name" value={form.studioName} onChange={(e) => setForm({ ...form, studioName: e.target.value })} />
                  <select className="input-field" value={form.craftType} onChange={(e) => setForm({ ...form, craftType: e.target.value })}>
                ...
                <div className="grid grid-cols-2 gap-3 mt-6">
                  <button onClick={() => setPage("explore")} className="btn-outline py-3">Maybe later</button>
        ...
        <div className="grid md:grid-cols-3 gap-3 mt-4">
          {[
            ...
            <div key={title} className="rounded-2xl p-4 border" style={{ background: "var(--cream)", borderColor: "var(--pink-border)" }}>
              <p className="text-2xl mb-3">{icon}</p>
              <p className="font-bold mb-1" style={{ color: "var(--charcoal)" }}>{title}</p>
...
function StudioPage({ setPage }: { setPage: (p: Page) => void }) {
  const { user, studioRegistered } = useApp();
  const myListings = PRODUCTS.filter((p) => p.active);

  // Gate: must be signed in to access studio
  if (!user) {
    return (
      <div>
        <TopBar title="My Studio" subtitle="Creator Workspace" />
        <div className="flex flex-col items-center justify-center py-24 px-6 text-center">
          ...
          <h2 className="text-xl font-bold mb-2" style={{ color: "var(--charcoal)", fontFamily: "Georgia,serif" }}>
    ...
    <div>
      <div className="px-6 pt-6 pb-7" style={{ background: "var(--sage-dark)" }}>
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider" style={{ color: "var(--sage-light)" }}>
            Creator Workspace
          </span>
        </div>
        <h1 className="text-3xl font-bold mb-1" style={{ color: "var(--cream)", fontFamily: "Georgia,serif" }}>
          Studio 138
        </h1>
      ...
      <div className="grid grid-cols-3 gap-px border-b" style={{ background: "var(--pink-border)" }}>
        {[
          ...
          <div key={label} className="flex flex-col items-center py-5 px-2 text-center" style={{ background: "var(--cream)" }}>
            <span className="text-xl mb-1">{icon}</span>
            <p className="text-sm font-bold" style={{ color: "var(--sage-dark)" }}>{value}</p>
            <p style={{ fontSize: "10px", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: "2px" }}>
              {label}
            </p>
          </div>
        ))}
      ...
      <div className="px-6 pt-5">
        <button className="w-full flex items-center gap-3 p-4 rounded-2xl mb-5 border" style={{ background: "white", borderColor: "var(--pink-border)" }}>
          <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: "var(--pink-bg)" }}>
      ...
      <div className="px-6 pt-5 pb-8 space-y-4">
        <div>
          ...
          <div
            ...
            style={{
              border: `2px dashed ${dragging ? "var(--sage)" : "var(--muted-light)"}`,
              background: dragging ? "rgba(234,247,197,0.25)" : "transparent",
            }}
          >
            {image ? (
              <img src={image} alt="product" className="w-full h-48 object-cover rounded-xl" />
            ) : (
              <>
                <Upload size={28} style={{ color: "var(--muted)" }} />
                <p className="text-sm text-center" style={{ color: "var(--muted)" }}>Click to upload or drag & drop</p>
                <p className="text-xs" style={{ color: "var(--muted)", opacity: 0.7 }}>High quality PNG, JPG up to 10MB</p>
              </>
            )}
          </div>
        </div>

        <input className="input-field" placeholder="Product title" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} />
        ...
        <textarea className="input-field resize-none" rows={4} placeholder="Describe your handmade item..." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />

        <div className="flex gap-3 pt-2">
          <button className="btn-outline flex-1 py-3">Save Draft</button>
          <button onClick={() => setPage("published")} className="btn-primary flex-1 py-3 gap-2">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: "var(--cream)" }}>
      <div className="w-full max-w-sm text-center animate-scaleIn">
        ...
        <h2 className="text-2xl font-bold mb-2" style={{ color: "var(--charcoal)", fontFamily: "Georgia,serif" }}>
...
function ProfilePage({ setPage }: { setPage: (p: Page) => void }) {
  ...
  return (
    <div>
      ...
      <div className="px-6 pt-5 pb-8 space-y-4">
        <div className="rounded-3xl p-6 text-center shadow-card animate-fadeIn" style={{ background: "white", border: "1px solid var(--pink-border)" }}>
          <div
          ...
          <h2 className="text-lg font-bold" style={{ color: "var(--charcoal)", fontFamily: "Georgia,serif" }}>
            {user?.username ?? "Guest"}
          </h2>
        ...
        <div className="rounded-2xl shadow-card overflow-hidden" style={{ background: "white", border: "1px solid var(--pink-border)" }}>
          {[
            ...
            >
              <div className="w-8 h-8 rounded-xl flex items-center justify-center" style={{ background: "var(--pink-bg)", color: "var(--pink-dark)" }}>
                {item.icon}
              </div>
        ...
        {user && (
          <button
            onClick={() => { logout(); setPage("explore"); }}
            className="w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl shadow-card"
            style={{ background: "white", color: "var(--rose)", border: "1px solid var(--pink-border)" }}
          >
            <LogOut size={18} />
...
export default function App() {
  ...
  const login = (u: AppUser) => {
    setUser(u);
    // If currently on login/signup, redirect
